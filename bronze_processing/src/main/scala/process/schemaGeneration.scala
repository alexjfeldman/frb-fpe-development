...
object schemaGeneration {
  ...
  def checkSchema (inputDF : DataFrame, tableName  : String, newColumnNames : Array[String], newColumnDataTypes : Array[String], userName : String, timeStamp : Long) : Boolean = {
    ...
    val checkSchemaDF : DataFrame =  spark.read.format("parquet").load("s3://frbmetadatadevelopment/Table_Schema_Check/*").filter($"Table_Name"===tableName).sort($"Column_Number")
    ...
    if(!runCheckBoolean) {
      ...
      fullSchemaDF.withColumn("Update_User_Name",lit(userName)).withColumn("Update_Time_stamp",lit(timeStamp)).write.format("parquet").mode("append").save("s3://frbmetadatadevelopment/Table_Schema_Check/")
